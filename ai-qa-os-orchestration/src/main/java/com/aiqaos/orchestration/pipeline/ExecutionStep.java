...
public class ExecutionStep implements WorkflowStep<WorkflowRequest, WorkflowResponse> {
    ...
    public WorkflowResponse execute(WorkflowRequest request, WorkflowContext context) {
        ...
        try {
            ...
            // 4. Resolve appropriate framework
            ...
            // 5. SCALE-1: run via the decoupled job queue when wired; otherwise the in-process engine (default).
            ExecutionResult execResult;
            if (executionJobQueue != null) {
                String workflowIdStr = context.getMetadata().getWorkflowId() != null
                        ? context.getMetadata().getWorkflowId().toString() : null;
                String executionIdStr = context.getMetadata().getExecutionId() != null
                        ? context.getMetadata().getExecutionId().toString() : null;
                com.aiqaos.execution.queue.ExecutionJob job = new com.aiqaos.execution.queue.ExecutionJob(
                        java.util.UUID.randomUUID().toString(), workflowIdStr, executionIdStr,
                        executionIdStr, framework, scriptSuite, config);
                executionJobQueue.submit(job);
                com.aiqaos.execution.queue.ExecutionJobResult jobResult =
                        executionJobQueue.awaitResult(job.getJobId(), java.time.Duration.ofMinutes(30));
                if (!jobResult.isSuccess()) {
                    throw new RuntimeException("Execution job failed: " + jobResult.getErrorMessage());
                }
