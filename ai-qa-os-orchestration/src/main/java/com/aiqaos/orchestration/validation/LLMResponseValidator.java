package com.aiqaos.orchestration.validation;
...
import com.aiqaos.orchestration.guardrail.ActionAllowlistGuardrail;
...
public class LLMResponseValidator {
    ...
    // SEC-3: output-grounding guard (null-safe — skipped if the bean is absent, e.g. direct construction).
    @Autowired(required = false)
    private ActionAllowlistGuardrail outputGuardrail;
    ...
    public String validateAndNormalize(AgentType type, String rawJson) {
        ...
        // SEC-3: reject output that echoes injection/hijack directives before the platform acts on it.
        if (outputGuardrail != null && guardrailsEnabled) {
            GuardrailVerdict verdict = outputGuardrail.check(
                    rawJson, GuardrailContext.output(type == null ? "llm" : type.name()));
            if (!verdict.isAllowed()) {
                if ("report".equalsIgnoreCase(guardrailsMode)) {
                    log.warn("[guardrail:output-grounding] report-only — {}", verdict.getReason());
                } else {
                    throw new IllegalArgumentException("Output grounding violation: " + verdict.getReason());
                }
            }
        }
        ...
        try {
            ...
            if (type == AgentType.TEST_CASE_GENERATOR) {
                ...
                // 3. Validate scripts array presence — the suite must come from the agent, never fabricated
                if (!objectNode.has("scripts") || objectNode.get("scripts").isNull() || !objectNode.get("scripts").isArray() || objectNode.get("scripts").size() == 0) {
                    throw new IllegalArgumentException("Validation failed: ScriptGeneratorAgent returned no scripts");
                }
