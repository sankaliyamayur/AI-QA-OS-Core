...
# Test Script for TC-004: Add Product to Cart
...
def test_add_product_to_cart(page: Page):
    ...
    home_page.add_item_to_cart(product_to_add)
    expect(home_page.cart_icon).to_have_text("1")

    home_page.go_to_cart()
    expect(page).to_have_url("https://www.saucedemo.com/cart.html")
    expect(cart_page.cart_items).to_have_count(1)
    assert product_to_add in cart_page.get_item_names_in_cart()
